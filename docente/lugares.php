...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($accion === "crear") {
        ...
        if ($nombre === "") {
            $error = "Ingrese el nombre del lugar.";
        } else {
            $sql = "INSERT INTO lugares (nombre, obs, usuario_id_creacion, fecha_creacion, hora_creacion)
                    VALUES (?, ?, ?, NOW(), CURTIME())";
            $stmt = mysqli_prepare($conexion, $sql);
            mysqli_stmt_bind_param($stmt, "ssi", $nombre, $obs, $_SESSION["usuario_id"]);

            if (mysqli_stmt_execute($stmt)) $mensaje = "Lugar registrado.";
            else $error = "No se pudo registrar el lugar.";
        }
    }
    ...
    if ($accion === "actualizar") {
        ...
        if (mysqli_stmt_execute($stmt)) $mensaje = "Lugar actualizado.";
    ...
    if ($accion === "eliminar") {
        ...
        if (mysqli_stmt_execute($stmt)) $mensaje = "Lugar eliminado lógicamente.";
        else $error = "No se pudo eliminar.";
    }
}

$lugares = mysqli_query($conexion, "SELECT * FROM lugares WHERE fecha_eliminacion IS NULL ORDER BY id DESC");

include __DIR__ . "/../includes/header.php";
?>

<section class="page-head">
    <h1>Lugares educativos</h1>
    <p>Registra los lugares donde el docente imparte clases.</p>
</section>
...
<section class="panel">
    ...
    <form method="POST" class="form-grid">
        ...
        <div>
            ...
            <input type="text" name="nombre" placeholder="Ej: Universidad / Colegio / Instituto" required>
        </div>
...
<section class="panel">
    ...
    <div class="table-wrap">
        <table>
            ...
            <tbody>
            <?php while ($fila = mysqli_fetch_assoc($lugares)): ?>
                <tr>
                    <form method="POST">
